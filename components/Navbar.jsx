import { assets } from '@/assets/assets'
import Image from 'next/image'
import React, { useRef } from 'react'

const Navbar = (props) => {

    const { ovoClass, outfitClass } = props

    const sideMenuRef = useRef();
    
    const openMenu = () => {
        sideMenuRef.current.style.transform = 'translateX(-16rem)'
    }
    
    const closeMenu = () => {
        sideMenuRef.current.style.transform = 'translateX(16rem)'
    }

    return (
        <>
            <div className='fixed top-0 right-0 w-11/12 -z-10 translate-y-[-80%]'>
                <Image src={assets.header_bg_color} alt="Header gradient background image" className='w-full' />
            </div>
            <nav className='w-full fixed px-5 lg:px-8 xl:px-[8%] py-4 flex items-center justify-between z-50'>
                <a href="#top">
                    <Image src={assets.logo} alt="Logo of the person" className='w-28 cursor-pointer mr-14' />
                </a>

                <ul className='hidden md:flex items-center gap-6 lg:gap-8 rounded-full px-12 py-3 bg-white shadow-xs bg-opacity-50'>
                    <li><a className={ovoClass} href='#top'>Home</a></li>
                    <li><a className={ovoClass} href='#about'>About me</a></li>
                    <li><a className={ovoClass} href='#services'>Services</a></li>
                    <li><a className={ovoClass} href='#work'>My Work</a></li>
                    <li><a className={ovoClass} href='#contact'>Contact Me</a></li>
                </ul>

                <div className='flex items-center gap-4'>

                    <button className='cursor-pointer'>
                        <Image src={assets.moon_icon} alt="toggle for theme mode" className='w-6' />
                    </button>

                    <a href="#contact" className={`hidden lg:flex items-center gap-3 px-10 py-2.5 border border-gray-500 rounded-full ml-4 ${ovoClass}`} >Contact <Image src={assets.arrow_icon} alt="Arrow to contacts" className='w-3' /></a>

                    <button className='block md:hidden ml-3 cursor-pointer' onClick={openMenu}>
                        <Image src={assets.menu_black} alt="Hamburger icon for menu toggle" className='w-6' />
                    </button>
                </div>

                {/* mobile menu */}

                <ul ref={sideMenuRef} className='flex md:hidden flex-col gap-4 py-20 px-10 fixed -right-64 top-0 bottom-0 w-64 z-50 h-screen bg-rose-50 transition duration-500'>

                    <div className='absolute right-6 top-6' onClick={closeMenu}>
                        <Image src={assets.close_black} alt="button to close the menu" className='w-5 cursor-pointer' />
                    </div>

                    <li><a className={ovoClass} onClick={closeMenu} href='#top'>Home</a></li>
                    <li><a className={ovoClass} onClick={closeMenu} href='#about'>About me</a></li>
                    <li><a className={ovoClass} onClick={closeMenu} href='#services'>Services</a></li>
                    <li><a className={ovoClass} onClick={closeMenu} href='#work'>My Work</a></li>
                    <li><a className={ovoClass} onClick={closeMenu} href='#contact'>Contact Me</a></li>
                </ul>
            </nav>
        </>
    )
}

export default Navbar